...
export namespace Tool {
    ...
    function write_loadsettingjs() {
        ...
        // 写入文件
        fs.writeFileSync(C.OUTPUT_LOADSETTINGJSON_JS, `  
    ...
    function modify_ccjs() {
        ...
        data = data.replace(/[a-z].loadNative[\s]=[\s]function[\s]*?\([a-z]\)[\s]\{[\s]*?return[\s]*?new[\s]*?Promise\(\(function[\s]*?\([a-z], [a-z]\)[\s]*?{[\s]*?var[\s]*?[a-z][\s]*?=[\s]*?lDe.getCache[\s\S]*?XMLHttpRequest[\s\S]*?"load audio failed:[\s\S]*?"arraybuffer"[\s\S]*?decodeAudioData[\s\S]*?send\(null\)[\s\S]*?\}\)\)[\s\S]*?\},/, 
        `e.loadNative = function (a) { return new Promise((function (t, n) { 
                            var i = lDe.getCache(a);
                            if (i) return lDe.retainCache(a), void t(i); 
                            externDownloadArrayBuffer(a,function(data){
                                oDe.decodeAudioData(data).then((function (n) {
                                    lDe.addCache(a, n), t(n)
                                })).catch((function () {}))
                            }) 
                        }))
                    },`);
